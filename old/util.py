...
class Vector:
    ...
    def cross(self, o):
        if not isinstance(o, self.__class__):
            raise NotImplementedError
        return self.__class__(np.cross(self._data, o._data), float)
...
class Matrix(Vector):
    ...
    @staticmethod
    def translate4(v, d, out = None, normalize = False):
        if normalize:
            v = v.normalize
    ...
    @staticmethod
    def translate3(v, d, out = None, normalize = False):
        out = Matrix.translate4(v, d, out, normalize)
        
        return Matrix(out.data[3, :-1])
    
    @staticmethod
    def rotate4(v, theta, out = None, normalize = False):
        if normalize:
            v = v.normalize
    ...
    @staticmethod
    def rotate3(v, theta, out = None, normalize = False):
        out = Matrix.rotate4(v, theta, out, normalize)
        
        return Matrix(out.data[:-1, :-1])
    
    @staticmethod
    def rotate_around4(v, theta, out = None, normalize = True):
        if normalize:
            v = v.normalize
        
        a = np.math.cos(theta / 2.0)
        b, c, d = -v * np.math.sin(theta / 2.0)
    ...
    @staticmethod
    def rotate_around3(v, theta, out = None, normalize = True):
        out = Matrix.rotate_around4(v, theta, out, normalize)
        
        return Matrix(out.data[:-1, :-1])
    
    @staticmethod
    def scale4(v, d = 1, out = None, normalize = False):
        if normalize:
            v = v.normalize
    ...
    @staticmethod
    def scale3(v, d = 1, out = None, normalize = False):
        out = Matrix.scale4(v, d, out, normalize)
...
class Color(Vector):
    def __init__(self, r, g = None, b = None, a = None, *, immutable = False):
        ...
        super().__init__(data, immutable = immutable)
        
        lower = 0. if is_float(self._data) else 0
        upper = 1. if is_float(self._data) else 255
...
WHITE = Color(1., 1., 1., immutable = True)
BLACK = Color(0., 0., 0., immutable = True)
RED = Color(1., 0., 0., immutable = True)
GREEN = Color(0., 1., 0., immutable = True)
BLUE = Color(0., 0., 1., immutable = True)
