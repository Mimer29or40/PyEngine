...
def random(lower=None, upper=None):
    ...
    lower, upper = (0.0, lower) if upper is None else (lower, upper)
...
class Connection:
    ...
    def __repr__(self):
        return "Connection<id={}, {}->{}, weight={}, enabled={}>".format(
            self.id, self.in_node, self.out_node, self.weight, self.enabled
        )
    ...
    def copy(self):
        return Connection(self.id, self.in_node, self.out_node, self.weight, self.enabled)
...
class Genome:
    ...
    def __repr__(self):
        return "Genome[nodes={} connections={}, fitness={}]".format(
            len(self.nodes.values()), len(self.connections.values()), self.fitness
        )
    ...
    def node_mutation(self, node_inno, connection_inno):
        ...
        suitable_cons = list(filter(lambda c: c.enabled, self.connections.values()))
        ...
        new_node = Node(id=node_inno.inc, layer=in_node.layer + 1, type=NodeType.HIDDEN)
        con1 = Connection(
    ...
    def save(genome, file_name="genome"):
        ...
        for node in genome.nodes.values():
            nodes[node.id] = {"layer": node.layer, "type": str(node.type).replace("NodeType.", "")}
        ...
        json.dump({"nodes": nodes, "cons": cons}, open(f"{file_name}.json", "w"), indent=4)
    ...
    def load(file_name, genome=None):
        ...
        for id, data in obj["nodes"].items():
            genome.add_node(Node(id=int(id), layer=data["layer"], type=NodeType[data["type"]]))
    ...
    def average_weight_diff(genome1, genome2):
        ...
        return weight_diff / count if count > 0 else 2.0
...
class Neuron:
    ...
    def __repr__(self):
        return "Neuron<inputs={} output={} outputs={} weights={}>".format(
            self.inputs, self.output, self.output_ids, self.weights
        )
...
class Evaluator:
    def __init__(self, settings, generator, evaluator, node_innovation, con_innovation):
    ...
    def evaluate_generation(self):
        ...
        if self.fittest_genome is None or champion.fitness > self.fittest_genome.fitness:
...
def fitness_plot(fitness, segments=4):
    ...
    return Image.fromarray(
        np.roll(
            np.fromstring(canvas.tostring_argb(), dtype=np.uint8).reshape(
