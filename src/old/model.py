    ...
    def bind_vertices(self, vertices, indices):
        ...
        for name in _attrs.keys():
            GL.glVertexAttribPointer(
                _locs[name], _attrs[name], GL.GL_FLOAT, GL.GL_FALSE, _v_len, _offs[name]
            )
...
class Material:
    def __init__(self, name):
        ...
        self.k_e = util.Color(0.0, 0.0, 0.0)
        self.t_f = util.Color(1.0, 1.0, 1.0)
        self.illum = 0
        self.d = 1.0
        self.n_s = 100
        self.sharpness = 60
    ...
    def bind(self, shader):
        shader.set_floatv("material.Ka", self.k_a.xyz.data)
        shader.set_floatv("material.Kd", self.k_d.xyz.data)
        shader.set_floatv("material.Ks", self.k_s.xyz.data)
        shader.set_floatv("material.Ke", self.k_e.xyz.data)
        shader.set_floatv("material.Tf", self.t_f.xyz.data)
...
class Texture:
    ...
    def gen(self, wrapS=GL.GL_REPEAT, wrapT=GL.GL_REPEAT, minFil=GL.GL_LINEAR, magFil=GL.GL_LINEAR):
