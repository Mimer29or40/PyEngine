...
class Engine:
    ...
    def _get_events(self):
        ...
        for e in pygame.event.get():
            if e.type == pygame.KEYDOWN:
                ...
                if k not in self._keys:
                    self._keys[k] = _Data(now, mod=e.mod, unicode=e.unicode, scancode=e.scancode)
                ...
                if b not in self._buttons:
                    self._buttons[b] = _Data(now, pos=Vector(e.pos))
                ...
                _Event(events, pygame.event.event_name(e.type), now, **e.__dict__)

        for k, d in self._keys.copy().items():
            if d.down:
                _Event(events, KEY_DOWN, now, key=k, mod=d.mod)
                self._keys[k].down = False
            else:
                _Event(events, KEY_HOLD, now, key=k, mod=d.mod)
            if d.up:
                if now - d.time < 1e9 / 5:
                    _Event(events, KEY_PRESSED, now, key=k, mod=d.mod)
                _Event(events, KEY_UP, now, key=k, mod=d.mod)
                del self._keys[k]
        ...
        for b, d in self._buttons.copy().items():
            ...
            if d.down:
                _Event(events, MOUSE_BUTTON_DOWN, now, button=b, pos=d.pos)
                self._buttons[b].down = False
            else:
                _Event(events, MOUSE_BUTTON_HOLD, now, button=b, pos=d.pos)
            if d.up:
                if now - d.time < 1e9 / 5:
                    _Event(events, MOUSE_BUTTON_PRESSED, now, button=b, pos=d.pos)
                _Event(events, MOUSE_BUTTON_UP, now, button=b, pos=d.pos)
                del self._buttons[b]
