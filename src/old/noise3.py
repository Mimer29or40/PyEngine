...
def to_normal(arr):
    x, y = np.meshgrid(*[np.linspace(0, dim, dim, endpoint=False, dtype=int) for dim in arr.shape])
...
class Noise:
    ...
    def generate_range(self, *ranges):
        ...
        for rng in ranges:
            ...
            lin_spaces.append(np.linspace(start, end, step, endpoint=False))
...
class PerlinNoise(Noise):
    ...
    def _calculate(self, dimension, coords, freq, amp):
        ...
        extrema = np.array(
            [[floor(values[i].min()), ceiling(values[i].max())] for i in range(dimension)]
        )
...
class WorleyNoise(Noise):
    def __init__(self, *, seed=1, octaves=1, persistence=0.5, count=4, correction=0.75, func=None):
        super().__init__(seed=seed, octaves=octaves, persistence=persistence)
    ...
    def _calculate(self, dimension, coords, freq, amp):
        ...
        extrema = np.array(
            [[floor(values[i].min()) - 1, ceiling(values[i].max()) + 1] for i in range(dimension)]
        )
...
if __name__ == "__main__":
    ...
    np.set_printoptions(linewidth=9999, precision=6, edgeitems=10, threshold=9999, suppress=True)
