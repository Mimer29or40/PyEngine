...
class Window:
    ...
    def screen_to_world(self, screen):
        if self.is_perspective:
            m = self.focal_length * np.math.tan(np.math.radians(self.fov / 2))
            screen = ((screen * 2 / self._screen) - 1) * m * 2
            screen.x *= self.aspect_ratio
            screen.y *= -1
            return self._focus + (self._r * screen.x) + (self._u * screen.y)
        else:
            pass
    ...
    def projection(self, aspect_ratio):
        if self.is_perspective:
            ...
            m22 = 2.0 / (self.z_near - self.z_far)
            m23 = 0.0
    ...
    def view(self):
        ...
        self._view.data = [
            [self._r.x, self._u.x, self._f.x, 0.0],
            [self._r.y, self._u.y, self._f.y, 0.0],
            [self._r.z, self._u.z, self._f.z, 0.0],
            [-r_dot, -u_dot, -f_dot, 1.0],
        ]
