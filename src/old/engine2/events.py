...
def get_events():
    ...
    for b, d in _buttons.copy().items():
        if _pos != _pos_prev:
            events.append(_Event(MOUSE_DRAGGED, now, button=b, pos=_pos, rel=_pos - _pos_prev))
        if d.down:
            events.append(_Event(MOUSE_BUTTON_DOWN, now, button=b, pos=util.Vector(d.pos)))
            _buttons[b].down = False
        else:
            events.append(_Event(MOUSE_BUTTON_HOLD, now, button=b, pos=util.Vector(d.pos)))
        if d.up:
            if now - d.time < 1e9 / 5:
                events.append(_Event(MOUSE_BUTTON_PRESSED, now, button=b, pos=util.Vector(d.pos)))
            events.append(_Event(MOUSE_BUTTON_UP, now, button=b, pos=util.Vector(d.pos)))
            del _buttons[b]

    if _pos != _pos_prev:
        events.append(_Event(MOUSE_MOTION, now, pos=_pos, rel=_pos - _pos_prev))
        _pos_prev = _pos.copy()

    if _scroll != _scroll_prev:
        events.append(_Event(MOUSE_SCROLL, now, pos=_scroll, rel=_scroll - _scroll_prev))
